...
//    }
//
//    // Always recursive; globで範囲制御
//    val paths: List<Path> = Files.walk(targetDir)
//        .filter { Files.isRegularFile(it) }
//        .collect(Collectors.toList())
//
//    val includeGlobs = dirAnn.includes.toList()
//    val excludeGlobs = dirAnn.excludes.toList()
//    val includeMatchers = includeGlobs.map { glob ->
//        FileSystems.getDefault().getPathMatcher("glob:${'$'}glob")
//    }
//    val excludeMatchers = excludeGlobs.map { glob ->
//        FileSystems.getDefault().getPathMatcher("glob:${'$'}glob")
//    }
//
//    fun matches(matchers: List<java.nio.file.PathMatcher>, p: Path): Boolean = matchers.any { matcher ->
